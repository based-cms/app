 ...
 * Full BetterAuth validation happens here — proxy.ts only does a lightweight cookie check.
 */
export async function assertOrgAccess(
  ...
  // BetterAuth puts the org ID in the JWT as `org_id` — matches the definePayload in auth.ts
...
/**
 * Returns the orgId from the current BetterAuth session.
...
export async function requireOrgId(ctx: QueryCtx | MutationCtx): Promise<string> {
  ...
  // BetterAuth JWT uses `org_id` — set via definePayload in auth.ts
